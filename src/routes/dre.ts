...
export default async function dreRoutes(fastify: FastifyInstance) {
  ...
  fastify.get("/dre/por-nivel1-e-nivel2-e-nivel3", async (request, reply) => {
    const { idorganizacao, modelo, ano, tipo } = request.query as {
    ...
    try {
      const resultado = await prisma.dre.findMany({
        where: {
      ...
      const agrupado: Record<string, Record<string, Record<string, { valores: { [mes: number]: number; total: number } }>>> = {};
      ...
      for (const item of resultado) {
        ...
        if (!agrupado[n1]) agrupado[n1] = {};
        if (!agrupado[n1][n2]) agrupado[n1][n2] = {};
        if (!agrupado[n1][n2][n3]) {
          agrupado[n1][n2][n3] = {
            valores: Object.fromEntries(Array.from({ length: 12 }, (_, i) => [i + 1, 0])) as any,
          };
      ...
      const dados = Object.entries(agrupado).flatMap(([nivel1, n2map]) =>
        Object.entries(n2map).flatMap(([nivel2, n3map]) =>
          Object.entries(n3map).map(([nivel3, obj]) => ({
            nivel1,
            nivel2,
            nivel3,
            valores: obj.valores,
